...
user.arrowEmpty();
user.regularFunctionNotation(); // regular function notation
user.shorthand(); // shorthand notation
// user.arrowEmpty()
user.arrowPassIn(["d", "f"]); // regular notation passing in params from outside
user.shorthandpassin(["d", "f"]);

```
### object with stored data
#### how data is stored, common format
```js
let user = {
  name: "crystal",
  age: 30,
